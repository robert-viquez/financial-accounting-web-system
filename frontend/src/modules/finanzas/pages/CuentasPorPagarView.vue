...
const { filters } = usePersistentFilters("cuentas_pagar_filters", {
  search: "",
  estado: null,
});
...
const headers = [
  { title: "Compra", key: "compra_numero" },
...
const pagosCuenta = computed(() =>
  pagos.value.filter(
    (pago) => pago.cuenta_por_pagar === cuentaSeleccionada.value?.id
  )
);
...
async function cargarCuentas() {
  ...
  try {
    ...
    const data = await fetchAllPages(getCuentasPorPagar, params);
    cuentas.value =
      filters.estado === "VENCIDA"
        ? data.filter((cuenta) => estadoCalculado(cuenta) === "VENCIDA")
        : data;
  } catch (error) {
    mostrarMensaje("No se pudieron cargar las cuentas por pagar.", "error");
  } finally {
    loading.value = false;
  }
}
...
async function cargarCatalogos() {
  const [mediosPagoResponse, pagosResponse] = await Promise.all([
    ...
    fetchAllPages(getPagosProveedores, { ordering: "-fecha" }),
  ]);
...
async function registrarPago() {
  ...
  try {
    await createPagoProveedor({
      cuenta_por_pagar: cuentaSeleccionada.value.id,
    ...
    mostrarMensaje("No se pudo registrar el pago.", "error");
  } finally {
    saving.value = false;
  }
}
...
async function anularPago(pago) {
  try {
    await anularPagoProveedor(pago.id);
    mostrarMensaje("Pago anulado correctamente.");
    await Promise.all([cargarCuentas(), cargarCatalogos()]);
  } catch (error) {
    mostrarMensaje(error.response?.data?.detail?.[0] || "No se pudo anular el pago.", "error");
  }
}
...
<template>
  <section>
    <PageHeader
      ...
      subtitle="Control de saldos pendientes, vencimientos y pagos a proveedores."
    />
        ...
        <v-row>
          <v-col cols="12" md="5">
            <v-text-field
              ...
              label="Buscar proveedor o compra"
      ...
      <v-card>
        ...
        <v-card-text>
          <v-row>
            <v-col cols="12">
              <v-text-field
                :model-value="cuentaSeleccionada?.proveedor_nombre"
                label="Proveedor"
                variant="outlined"
                density="compact"
                readonly
