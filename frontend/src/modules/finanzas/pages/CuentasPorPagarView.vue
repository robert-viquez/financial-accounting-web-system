...
async function cargarCuentas() {
  ...
  try {
    ...
    const response = await getCuentasPorPagar(params);
...
async function cargarCatalogos() {
  const [mediosPagoResponse, pagosResponse] = await Promise.all([
    ...
    getPagosProveedores({ ordering: "-fecha" }),
  ]);
...
async function confirmarEliminar() {
  ...
  try {
    await deleteCuentaPorPagar(cuentaSeleccionada.value.id);
...
<template>
  <section>
    ...
    <ConfirmDialog
      ...
      :message="`¿Desea eliminar la cuenta ${cuentaSeleccionada?.compra_numero || ''}?`"
